...
export function PreviewPanel({
  ...
  useEffect(() => {
    ...
    return () => {
      // Unsubscribe not directly exposed by API types usually
    };
  }, [webcontainer]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0 w-full relative overflow-hidden bg-slate-950">
        ...
        <div className={cn("h-full w-full overflow-auto bg-[#011627]", viewMode === 'code' ? 'block' : 'hidden')}>
          {activeFile ? (
            <Highlight
            ...
            >
              {({ className, style, tokens, getLineProps, getTokenProps }) => (
                <pre className={cn(className, "p-4 text-xs font-mono min-w-full bg-transparent")} style={style}>
                  {tokens.map((line, i) => {
                    const { key: lineKey, ...lineProps } = getLineProps({ line, key: i });
                    return (
                      <div key={i} {...lineProps}>
                        <span className="inline-block w-8 text-slate-600 mr-2 text-right select-none">{i + 1}</span>
                        {line.map((token, keyIdx) => {
                          const { key: tokenKey, ...tokenProps } = getTokenProps({ token, key: keyIdx });
                          return <span key={keyIdx} {...tokenProps} />;
                        })}
                      </div>
                    );
                  })}
