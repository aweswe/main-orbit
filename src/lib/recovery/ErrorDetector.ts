...
export class ErrorDetector {
    detect(error: string): ErrorType {
        return ErrorDetector.detectErrorType(error);
    }
    ...
    static detectErrorType(error: string): ErrorType {
        ...
        if (errorLower.includes('command not found') || errorLower.includes('not recognized as an internal')) {
            const cmdMatch = error.match(/([^:\s]+): command not found/i) || error.match(/'([^']+)' is not recognized/i);
            return {
                type: 'missing-dependency',
                file: cmdMatch ? cmdMatch[1] : undefined,
                message: error,
        ...
        if (errorLower.includes('cannot find module') ||
            ...
            return {
                ...
                line: this.extractLineNumber(error),
                message: error,
                suggestedFix: 'Check import path or install missing package'
            };
        }

        // Circular Dependency
        if (errorLower.includes('circular dependency') ||
            errorLower.includes('cyclic dependency') ||
            error.includes('Circular import')) {
            return {
                type: 'circular-dependency',
                file: this.extractFilePath(error),
                message: error,
                suggestedFix: 'Refactor to break circular import chain'
            };
        }

        // CSS Syntax Errors
        if (errorLower.includes('postcss') ||
            errorLower.includes('css syntax error') ||
            error.includes('tailwindcss') && errorLower.includes('error')) {
            return {
                type: 'css-syntax',
                file: this.extractFilePath(error) || 'index.css',
                line: this.extractLineNumber(error),
                message: error,
                suggestedFix: 'Fix CSS syntax or Tailwind directive'
            };
        }
